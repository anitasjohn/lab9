...
void insertion_sort(int *a, int n)
{
    ...
    for (i = 1; i < n; i++) {
        value_to_insert = a[i]; // 5
        
        /* Shift values greater than value_to_insert. */
        j = i; //j = 5 = i
        while ( j > 0 && a[j - 1] > value_to_insert  ) { // 5 > 0 && a[4] = 308 > 413 ----- 4>0 && a[3] = 171 > 
            a[j] = a[j - 1];
            j--;
        }
        
        a[j] = value_to_insert; // a[5] = 413
    }
}
void insertion_sort(const char** str_array, int n){
    int i;
    ...
    for (i = 1; i < n; i++) { 
        value_to_insert = str_array[i];
            ...
            j = i; //j = 5 = i
        while ( j > 0 && strcmp(str_array[j-1],value_to_insert)>0 ) { // 5 > 0 && a[4] = 308 > 413 ----- 4>0 && a[3] = 171 > 
